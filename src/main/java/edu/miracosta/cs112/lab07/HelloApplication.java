...
import javafx.scene.control.TextField;  //class for textfield
...
//        layout.getChildren().add(button);
        layout.getChildren().addAll(message,name,button);

        //SETUP SCENE AND SHOW
        Scene scene = new Scene(layout, 300, 250); //layout, dimensions of window
        primaryStage.setScene(scene);
        primaryStage.setTitle("Sam Jorgensen"); //setting title of main window
        primaryStage.show();
    }
    ...
    public void handle(ActionEvent actionEvent) { //generic method used to handle when events occur (like handle button click)
        ...
        if(actionEvent.getSource() == button) {
            ...
            //update label
            message.setText("Hi, #" + count+ ", "+name.getText()+"!");
